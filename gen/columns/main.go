...
func genColumns(
	...
	if _, err := fmt.Fprintln(w, "// Code generated by gen/columns/main.go. DO NOT EDIT."); err != nil {
		return err
	}
	...
	}
	{
		for _, dimension := range response.Dimensions {
			...
			if _, err := fmt.Fprintf(w, "// Dimension%s : %s\n", title, dimension.Description); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "Dimension%s = %q\n", title, dimension.ApiName); err != nil {
				return err
			}
		}
		for _, metric := range response.Metrics {
			...
			if _, err := fmt.Fprintf(w, "// Metric%s : %s\n", title, metric.Description); err != nil {
				return err
			}
			if _, err := fmt.Fprintf(w, "Metric%s = %q\n", title, metric.ApiName); err != nil {
				return err
			}
		}
	}
